...
AABB is the simplest bounding volume that can be applied in 3d Euclidean space. AABB is the minimum perimeter along the directions of the axes that span the Euclidean space that fully contain the target object in 2d space as seen in Figure \ref{fig:aabb_obb}. AABB is straightforward to apply in 3d by finding minimae and maximae of the point set along each axis and by spanning a convex polyhedron so that all the minimum values are selected as the other corner that spans the polyhedral graph, and all the maximum values are selected as the opposing corner of the polyhedral graph.

\begin{figure}[ht]
  \begin{center}
    \includegraphics[width=\textwidth]{aabb_obb.jpg}
    \caption{a) Axis-aligned bounding box perimeter b) Oriented bounding box perimeter}
    \label{fig:aabb_obb}
  \end{center}
\end{figure}

AABB computation is very simple, but the biggest drawback of this technique is the large amount of empty space not occupied by the object that the AABB bounding volume contains. In the special case of maximally off-axis aligned pole shaped objects, or planar objects, an AABB volume representation may report a volume that is occupied by empty space up to more than 95\% of the volume.

To combat the AABB reported bounding volume as function of orientation, an oriented bounding box can be used instead. An oriented bounding volume solution rotates the axes of the perimeter so that the minimum bounding volume is found and being reported as seen in Figure \ref{fig:aabb_obb}. OBB will help in reporting the correct minimum bounding volume of specially shaped objects, such as cylinders and planar items. It also computes the relative orientation of the load object in the sensor frame.

It is possible to find more complex bounding volume techniques for even more accurate object volume representation, such as bounding volume shapes, bounding volume hierarchies (BVH), discrete oriented polytopes (DOPs), k-DOPs and boxtrees. More complex representation of object perimeters can lead to more accurate collision detection techniques, for example, BVH can be used to detect collisions or object interference using raytracing and culling. In the load object measurement purposes, an AABB bounding volume is a good solution, and OBB bounding volume is left as an improvement suggestion in the current software solution.
