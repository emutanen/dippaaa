...
The design of the implemented load object position and orientation system did evolve during its implementation cycle. The original design was non-linear, component-based design running on C++ code base using the ROS system. After it was proofed that the Point Cloud Library processing could be used together with OpenCV processing, the integration of the ROS system started. The final implementation is now more linear, component-based design, still running on ROS platform as separate node entities. Since the amount of data generated in the stereo cameras was quite large, the linearity of processing components was used to enforce the design where data decreases as its information content increases.
