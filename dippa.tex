...
Industry applications for machine vision systems can be found from very different fields ranging from space exploration to agriculture, and from mobile robotics to quality control on a manufacturing site. The use of machine vision brings the speed of the computer to many errands that are dangerous or impossible for a human to do, for example quality control in high speed manufacturing line processes in a paper factory, or  
...
The point cloud data quality was affected by the overall machine vision process including stereo calibration, disparity tuning, and the hardware setup. The hardware setup and the stereo calibration were used as monolithic initial calibration items. The calibration was done automatically for the hardware setup, and after a good basic point cloud output generation was achieved, no further engineering was done for the hardware or the stereo calibration. After an initial point cloud output is generated, the problem of its data quality can be presented: how well does the point cloud data resemble the real world scene? \par
Some point cloud data quality metrics can be found from literature, such as spatial structure analysis methods and positioning goodness in registration schemes \ref{Feng08}. Normally these methods are used in modeling of urban constructed environments and in airborne LIDAR mapping, which encourages to try their suitability for measuring constructed objects in indoor environments, too.\par
In spatial structure analysis, the features of the real world are measured, and their data responses are verified to hold the same qualities: e.g. position, orientation, volume, and surface. A suite of 3 tests that verify the point cloud data quality in the spatial structure sense may be formulated now.
For position verification, landmark objects may be installed in the environment, and their correct locations can be searched and verified in the point cloud data later. For orientation verification, eigen vectors may be calculated for a point cloud describing the feature and the largest eigen vector direction should be compared to the real-world object main axis. For volume verification, an oriented bounding volume should be computed for objects that can have a meaningful volume, and be compared to the real-world object.\par
Positioning goodness was not analysed in this thesis since no multiple view registration was done. Still, the spatial location verification with landmarks or beacons would be useful since it could be used to verify that the scale of the point cloud is correct, and all the measured lengths are actually correct. Since the data set environments did not contain any locations landmarks, the scale verification could not be done. A further development suggestion would be to add landmarks objects (e.g. standard size boxes) in the environment and measure their locations prior to capturing the data sets to be able to do spatial structure analysis.\par
A single effective spatial structure analysis that was possible to do with the current offline data sets, was to check how big is the angle between standing cabinets and the floor on the process crane hall floor. For example, a cabinet might be known to form a 90 degree angle with a floor, and such a scene would generate a point cloud response where the angle is not exactly 90 degrees (it could be e.g. 85 degrees).
In literature, also reverse engineering of shape primitives have been used for point cloud data quality verification in industrial settings. In industrial environments, a lot of box-shaped or cylinder-shaped objects can be found and be used for verification \ref{Fidera04}.





Additionally, it is possible to measure locations of landmarks, measure goodness of a fit of a primitive shape, and measure lengths of objects that the scale of the point cloud may affect.\par
It is not meaningful to think of point cloud data quality if the scene is unknown, but for example a goodness of fit for a primitive shape is simple to quantify. It is easier to measure the goodness of the data quality for scenes with previously known objects, which shall be known as standard objects. In the process hall data sets, a single standard object exists that is a large box of dimensions 1.52 x 0.94 x 0.92 meters. The object is constructed of metal sheets, and it is a box-shaped, partially see-through mass weighing in 3.97 tonnes. In the forestry crane measurements, no standard objects were used, and as such, they will not be used for certifying the point cloud data quality. 
...
Spatial structure analysis in other research has been carried out using spatial 3d markers or standard objects that can be located and measured in the output point cloud. Unfortunately, markers were not incorporated in the recorded offline data sets, which is why the spatial structure analysis for offline data sets are only carried out using standard object tests. The standard objects were used and measured in the overhead crane case, and in the laboratory environment.\par
One standard object measurement test was carried out in the laboratory environment for which the image data set reported...
Another standard object measurement test was computed using the overhead crane case, which reported object lengths of...



The machine vision process generated a point cloud that illustrated the 3d real-world scene viewed by the stereo cameras. The point cloud was constructed from the image areas that were visible for both cameras, and could be modeled according to section \ref{section:depth_map_generation}. For some scenes the point cloud data resembled the real thing, and for some scenes the point cloud did not look like what it was supposed to look like. The problem is how can this resemblance of the real scene be quantified by a machine? Is it possible to automatically get a value of how good the data describing the real scene is? This section tries to formulate a coherent goodness measure for the output point cloud quality where most important factors of point cloud quality are 
