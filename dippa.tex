...
In the modern world, goods are being moved more than ever before, which brings challenges to keeping the goods handling easy and safe. Products are mostly transported from one location to another using a modern automatic container handling process, but for example loading and unloading events remain laborous when single industrial items, such as engines or other machinery, are moved to a new location. Usually an overhead crane is used to do heavy lifting in a warehouse. Many types of different overhead cranes and rotary boom cranes have been developed to do the heavy lifting work, but currently the electric overhead crane (EOC) is the de facto standard for lifting goods in indoor warehouse environments.
...
as requirements for speed, safety, and performance of the overall production increases, it is possible that the crane operation itself will start to bottleneck the overall performance of an industrial 


which is the primary equipment whose operation can bottleneck the 
