...
The Himmeli platform includes three modules that can be used separately or together. Namely they are the sensor module, a computation module, and a support module with an IR light source seen in Figure \ref{fig:himmeli_modules}. Since most overhead cranes have little available space to install anything, only the sensor module was used and installed during overhead crane testing sessions. The largest frame on the backside of the rig is the computation unit including two PC computers and a control cabinet. In front, the sensor module includes a high resolution stereo camera pair, a single high resolution camera, a thermal imaging camera, an automotive ultrasound RADAR, and an inertial measurement unit (IMU).
...
The Himmeli platform computation module includes different components that control the power and the data flow of the perception platform. The main components are the integrated PC computers. The other PC controls the camera software that runs in a Windows 7 operating system, and the other runs all the image processing tasks using Robot Operating System in a Linux distribution. The platform runs on 230 volts AC power with an option for 24 volts DC operation. 
