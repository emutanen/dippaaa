...
The load object measurement implementation was realised as part of the FAMOUS project in Generic Intelligent Machines (GIM) research group. The implemented software included e.g. the stand-alone load object measurement tool that processes point cloud data into load object bounding volume information, and a point cloud data publisher that generates data using the Himmeli stereo camera platform.

The programming work was carried out between March 2013 and May 2014. In addition to standard C and C++ libraries, external libraries specialized in image processing and point cloud processing were used. OpenCV library was used for image processing and stereo rectification purposes, and Point Cloud Library (PCL) was used for all data intensive point cloud processing. Boost library was used for all file system operations. All the data networking, runtime configurations, and other networking solutions were implemented using the Robot Operating System (ROS).
...
The Himmeli platform includes three modules: the sensor module, a computation module, and a support module with an IR light source (see Figure \ref{fig:himmeli_modules}). On the front side of Himmeli, the sensor module includes a high resolution stereo camera pair, a single high resolution camera, a thermal imaging camera, an automotive ultrasound RADAR, and an inertial measurement unit (IMU). There is an optional Velodyne HDL-32E LIDAR sensor on top of the sensor module. The Velodyne HDL-32 can be used to scan the environment geometry with 32 micromirror controlled lasers. The schematic of all the sensors can be seen in Figure \ref{fig:himmeli_schematic}.

The automotive radar installed in Himmeli was not used at all due to its low resolution output, but it could be helpful under adverse weather conditions\citep{Ahtiainen12}. If the load object measurement system was used to measure distance of container cranes in a seaport terminal, then the RADAR would be used during low visibility periods of operation.
