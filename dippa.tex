...
where $f$ is the focal length of the parallel stereo camera pair, $b$ is the baseline of the stereo pair, and $d$ is the measured disparity shift between the stereo images. As seen from the result \ref{eq:depth_solution2} when disparity is zero, the depth output is at infinity ($z = \inf$), and there is virtually no shift visible in the image between the viewpoints.
