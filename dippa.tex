...
The Himmeli platform includes three modules that can be used separately or together. Namely they are the sensor module, a computation module, and the infrared light source module. Most overhead cranes have little available space to install anything, so only the sensor module was installed in the overhead crane. The largest frame on the backside of the rig is the computation unit visible in Figure \ref{fig:himmeli_platform}, including two PC computers and a control cabinet. In front, the sensor module includes a high definition stereo camera pair, a single high definition camera, a thermal imaging camera, and an automotive ultrasound RADAR.

The automotive radar installed in HIMMELI cannot be used to model the environment because of its low accuracy. In general, radars are not often used in machine perception systems due to their low resolution output. They can operate in adverse weather, such as fog, so some applications for the radar can be found in outdoor environments e.g. in seaport container crane measurement. A radar can measure environmental properties in the range of 1 to 120 meters, and up to a maximum accuracy of 5 cm on 5GHz bands. \cite{Ahtiainen12}.
