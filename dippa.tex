...
In a simple stereo camera configuration the depth of a measured feature can be calculated if the geometry of the stereo setup and the intrinsic camera parameters are known. The depth calculation is done according to equation \ref{equation:disparity_eq},

\begin{equation}
\label{equation:disparity_eq}
\mathsf{depth} = \frac{\mathsf{bf}}{\mathsf{d_x}}
\end{equation}

where \emph{b} is the baseline width between the principal points of the image planes, f is the focal length for both cameras, and $d_x$ is the horizontal disparity value measured.
