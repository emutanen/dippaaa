...
Before the object segmentation results in Figures \ref{fig:downsampling} and \ref{fig:object_segmentation} are possible, the ground plane must be removed from the original data (see ground plane in Figure \ref{fig:object_segmentation} a)). In order to find and remove the ground plane in the software system, parameter estimation technique was used.

Parameter estimation finds models from datasets, and in this case, it is known that the parametric model for the ground plane is a planar model, which can be equated as 
...
The parameter estimation tries to fit the model of Equation \ref{eq:planar_model} in the dataset using PCL sample consensus model types and methods. Other model types include planes, cylinders, spheres, and other simple models that can be formulated using an implicit parameter model. The existence of a certain model in the dataset is tested by finding inliers and outliers that determine whether the data set supports the model or not for some initial guess. A typical sample consensus (SAC) search may include hundreds of iteration, and the number of maximum iterations is usually limited. 
