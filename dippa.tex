...
\begin{figure}[ht]
  \begin{center}
    \includegraphics[width=\textwidth]{enlarging_effect.jpg}
...
In dataset 1372842898 the overhead crane was moving while stereo images were captured, which introduced problems with the quality of the scene geometry. It was noticed that the floor plane geometry showed large deviations in the reprojected depth value between the frames. It was also concluded that the perception platform moves between the left and right camera shutter exposures, which causes the reprojection error of the ground plane. The \emph{Himmeli} platform software camera shutter synchronization introduces random delays between camera exposures since the deviations in ground plane level showed randomly too little and too large values.
