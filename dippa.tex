...
3d data geometry was assumed correct with a calibrated stereo system, which is an assumption that may or may not be true. 3d data geometry was verified only for load objects by comparing the dimension measurements produced by the load object measurement software and the actual measure taken with a measuring tape. This was not enough to validate any other parts of the geometry in the surrounding environment.

A suggested improvement for environment 3d modeling is to install spheres as control points in the environment before data capture. Then, the reconstructed geometry can be verified as a function of distance from the cameras by comparing the computed locations of the spheres with their actual locations. In this way, it is possible to understand if the scale of the 3d geometry is incorrect, or some dimension is distorted. In this thesis, the emphasis was not on environment modeling, thus, the 3d geometry of the environment was not verified, and as long as the load object reports realistic dimensions, it can be .


The values produced by the software were not comparable with the actual measurements before the orientation of the sensor frame and rotation of the load object were taken into account. The axis-aligned bounding volume reported on average approximately 5-10\% larger values than the actual values of the dimensions, which is an acceptable result that can be used in a route planner. It was also deduced that a single measurement will not report the correct bounding volume, but it is a better idea to report the median or average bounding volume of a series of e.g. 10 to 15 measurements. Since the Himmeli platform can successfully deliver 4 frames per second without skipping frames, a load object measurement would take 3-5 seconds of time in total with the used software. Since cameras can deliver images at a rate of 30 FPS, the software is the bottleneck currently. Software operations can be sped up by removing visualisation from the software, or by using lower resolution cameras, or by using more parallel programming and GPU implementations for point cloud processing. Since the
