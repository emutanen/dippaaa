...
The pinhole camera is a simple box that has an ideal pinhole aperture on one of its sides, see Figure \ref{fig:pinhole_camera}. The camera forms an upside down image of a 3d object located at a distance of $f$ from the ideal camera focal point (pinhole) on its imaging plane $\pi$. An optical axis $I_o$ (dashed line in Figure \ref{fig:pinhole_camera} that is in the positive Z axis direction in the camera frame $O_c$) lies perpendicularly to the image plane pointing towards the object. 

Figure \ref{fig:central_projection} shows a \emph{central projection model} that is often used to simplify formulation of the mathematics in the pinhole camera image formation. In a pinhole camera, distance between the ideal lens and the image projection is $-f$, and the projection is behind the focal point, whereas the central projection model forms the image in front of the focal point, which is a center of projection. The only difference between the models is that the pinhole camera model forms a real image that is upside down, and the central projection model forms a virtual image in front of the lens. Using the simpler central projection model, the object and its projected image are related by a thin lens equation
