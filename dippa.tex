...
Log measurement using simple cylinder fitting was implemented successfully, and the resulting cylinder extraction result can be seen in Figure \ref{fig:log_detected}. The orientation of the log was found approximately within a 10 degree error window 95\% of the time, but no exact statistics were computed regarding correctness of the extracted load object orientation. The position of the log object includes a translation error in the direction of the cameras that is caused by the missing data points from the back side of the log. The amount of erroneous translation towards the cameras was not measured. Both the translation error and the orientation error affect the bounding volume measurement output, but only minor error is generated by these errors.

\begin{figure}[ht]
  \begin{center}
    \includegraphics[width=\textwidth]{log_detected.jpg}
    \caption{Log objected detected using data from BM algorithm in block matching (low quality data).}
    \label{fig:log_detected}
  \end{center}
\end{figure}

Better heuristic checks for correct load object selection is suggested as an improvement in the log object extraction and measurement processing. Currently, the correct log is extracted most of the time, and selecting parts of the crane boom is prevented using simple constraints, such as, preventing selection of cylinders found above the end effector. Still, it is possible to select a wrong load objects that is located below the end effector, for example, data generated by a pile of logs. Two checks that would improve the current cylinder selection significantly would be 
...
For example, if the distance between the candidate cylinder and the end effector is too large, then the candidate should be rejected. An improved load selection algorithm would be easy to implement, but due to limited time resources it was not further developed.

In Figure \ref{fig:load_detected1}, the stereo parameters and object segmentation parameters were not tuned optimally. Still, the segmentation was quite robust in presence of missing data, noisy 3d data, and non-optimal tuning of parameters. Especially the OpenCV stereo calibration that was described in Section \ref{section:stereo_calibration} will estimate good parameters with little a priori information, and work a lot towards robust functionality.

The most significant problem in the log object measurement was the grouping of multiple objects in the load object point cloud. While the cylinder extraction does a good job of extracting the correct cylinder, it remains unknown whether the object segmentation step is needed at all in forestry crane case since objects tend to group in a single large point cloud most of the time. The load object point cloud usually contains the crane boom, the end effector tool, and the picked up log. Sometimes, the load object point cloud additionally contained objects lying on the ground.

It could be assumed that the load objects are always found below the end effector tool, which would limit the number of points needed to process, but it can be easily shown that in normal operation the log is often partially seen above the end effector tool. In forestry crane test site, the long logs did easily tilt and reach above the end effector tool location when the operator picked them up. Consequently, the bounding volume does reach above the end effector height, and the end effector coordinate will remain inside the load object bounding volume. It was decided that in order to avoid accidental cutting of the load object data, the assumption of a load object located only below the end effector tool cannot be used. Consequently, only object centroid proximity information should be used to decide whether the selected load object is actually grabbed by the end effector tool or not.

There are no major improvement suggestions for the log object measurement process. One improvement option would be to implement a more complex cylinder structure that would better approximate the shape of the log object. A model that approximates the log object with partially continuos segments of a cylinder is introduced in Forsman's thesis \citep{Forsman10}. At this point, a simple cylinder with a linear axis is used, because a more complex log object model would significantly slow down the computation, and the effect on the bounding volume is once again minimal.
...
A lot of time was used in development of the load object measurement software. The software structure follows a standard ROS package implementation, and most of the required computation algorithms were readily available as part of OpenCV and PCL software libraries. The software program runs some advanced algorithms, such as RANSAC computation and k-nearest neighbour search, but these are not visible for the user. It would have been possible to implement some portions of the software better than what is currently implemented, for example, included an end effector tracker in the overhead crane case. Due to limited resources most computations use external libraries, and only some heuristics and a point cloud projections were implemented by the author. 
%The suggested improvements for the load measurement software were reported in more detail in Chapter \ref{}.

Other reasons for good robustness are the smart handling of uninitialized data in PCL library, and the software architecture that does not differentiate X, Y, and Z directions - no information about orientation of the scene is utilized in 3D segmentation. 
