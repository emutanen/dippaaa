...
Measuring goods while they are being moved and processed at the industrial site is the very basis for all quality control: RFID tags are used to identify and control a batch of products in bulk manufacturing, video systems measure process quality in a paper factory, and spectral analyzer measures the concentration of foreign substances in a soda bottle washing process. While some fast, optimized, profitable and safe system examples are found, many industrial ventures still rely on manual movement of a product on a factory floor. Manual operating can suffer from non-optimized route selection, operator fatigue, damage to goods in collisions, safety issues for other workers, and other problems due human errors. Now, if a real-time description of the load object and its environment was available, it is possible to prevent such problems using the installed crane systems itself.

A semi-automatic safe process environment is the desired target that an accurate description of the moved item and its environment could provide. This approach can bring extended safety with collision avoidance scheme, and real-time information for a remote user on currently moving goods, and poeple movement. The main non-safety related feature is the optimized movement of goods, which can save a lot of time and energy during the life cycle of the process hall crane.
...
If only an external dimension measurment is installed 
...
Industrial goods, such as beams, engines, large coils, cylinders of dangerous goods, and pallets of stacked items are mostly being moved using manually operated cranes. A lot of functionality has been added to the crane to help the operator work in safe conditions, for example, wireless remote controllers for freedom of movement, anti-sway pulley systems, and others. One problem is that while the operator can move an item safely to its destination, 
