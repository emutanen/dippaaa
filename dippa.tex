...
ROS Groovy was the version used for all development purposes, and packages such as rqt\_graph, rqt\_logger, rviz, dynamic\_reconfigure, and others were used for debugging the software. At the heart of the ROS system there is a ROS parameter server, which is detailed next.
...
ROS uses parameter server that keeps track of all topics, nodes, and parameters used in the network. The parameter server handles all data relay information, and the names used for all parameters have to be unique to prevent namespace collisions in the parameter server. Unique naming of parameters was enforced during the work by using a long naming convention where each parameter included the node name also in the beginning of the parameter name. 
