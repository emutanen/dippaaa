...
Before the object segmentation results in Figures \ref{fig:downsampling} and \ref{fig:object_segmentation} are possible, the ground plane must be removed from the original data (see ground plane in Figure \ref{fig:object_segmentation} a)). If the data points belonging to the ground plane are removed, then only the load object and other items in the environment will remain in the point cloud data. In order to find and remove the ground plane a parameter estimation technique was used.

Parameter estimation finds models from datasets, and in this case, it is known that the parametric model for the ground plane is a planar model (in 3 dimensions), which can be equated as 
...
The parameter estimation tries to fit the model of Equation \ref{eq:planar_model} in the test data using PCL sample consensus model types and methods. Other model types include planes, cylinders, spheres, and other simple models that can be formulated using an implicit parameter model. The existence of a certain model in the dataset is tested by guessing an initial model, and finding out inliers that determine whether the dataset supports the candidate model or not. A typical sample consensus (SAC) search may run hundreds of iterations, and the number of maximum iterations is usually limited. 

Classic parameter estimation methods (e.g. least squares minimisation) effectively optimise the fit of the selected parameter model using all points in the data set. In 1981, Fischler and Bolles presented a new parameterised model estimation technique called random sample consensus (RANSAC) that improves classic techniques by detecting which points are outliers in the data\citep{Fischler81}.

While classic parameter estimation techniques incorrectly assume that most outliers are smoothed out in favor of good results, the RANSAC technique initialises the inlier point search with a small initial data set, and accumulates inliers when consistent data points are found. This way, outlier data points are not used in the model fitting, and the solution is found easily even when gross errors are present in the dataset.
