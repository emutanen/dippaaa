...
Cranes have been used for moving goods as long as people have been relocating from one place to another.

In the modern world, we still move goods and we wish to make it as easy, cost-efficiently, and safe as possible.
The bulk of moving products are shipped to the customers all over the world through a complex product shipment processing.
+ lifting of heavy objects => containers 
...
%================================================================================================================================
\chapter{Environment Acquisition, Representation, and Object Modeling}
\label{chapter:environment_acquisition_representation_and_object_modeling}


\subsection{Commercially Available Sensors}
...
\section{3d Scene Representation}
...
\emph{rqt} visualizer can be used for subscribing to topics, and for visualizing the data contained in them. For example, we may verify that both stereo camera image feeds are actually online, and see how the computed point cloud looks like nearly in real-time.


limitations: range-based noise 
...
%end of chapter 3
\chapter{Stereographic Machine Vision System}
...
\section{Camera Modeling And Calibration According To Pinhole Camera Model}
\label{section:camera_calibration_according_to_pinhole_camera_model}

\subsection{Available Camera Models}
\label{subsection:available_camera_models}
...
\chapter{Load Object Registration And Tracking}
...
\section{Load Object Registration}
\label{section:load_object_registration}

\subsection{Multiple Viewpoint Registration}
\label{subsection:multiple_viewpoint_registration}

\section{Load Object Tracking}
\label{section:load_object_tracking}
...
\label{chapter:implementation_and_algorithms}
...
\subsection{Platform Installations}
...
\subsection{Point Cloud Processing Using Point Cloud Library}
\label{subsection:point_cloud_processing_using_point_cloud_library}

\subsubsection{Object Segmentation}
\label{subsubsection:object_segmentation}
...
\subsubsection{Geometric Fitting}
...
First, the tuning was started with a coarse selection of parameters to get a working feature matching response. Especially, the SAD window size was kept higher than normal to produce a lot of visible matched points. The most important parameters in the final BM state tuning set are SAD window size, number of disparities, and speckle range parameters. These parameters should be tuned first, and other parameters can be used for fine tuning of the disparity map afterwards. 

The SAD window size, or sum of absolute differences window size, affects the size of the sliding kernel window used for feature matching in the 1d search space for rectified images. The smaller the value, the smaller the feature smoothing kernel used is, and with a minimum value the search is called pixel-to-pixel feature matching. For metric measurement purposes the disparity map should be as accurate as possible, thus, the minimum value of the SAD window size will be used at all times. The downside of high accuracy disparity map is that the number of matched points is smaller than with less accurate tuning sets. If the point cloud is used for 3d visualization purposes, a higher SAD window size value selection should produce more points for the delight of the viewer.

The number of disparities parameter sets the number of disparity levels in a disparity map image. The higher the value, the 
...
\hline
Pre-Filter Size & 5 & 255 & Odd Number \\
\hline
Pre-Filter Cap & 1 & 63 & - \\
\hline
