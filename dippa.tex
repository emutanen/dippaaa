...
\section{Monocular Camera Calibration}
\label{subsection:monocular_camera_calibration}
...
Monocular camera calibration is the process of finding unknown camera model intrinsic parameters e.g. the focal lenght components for a single camera system. Using the pinhole camera model introduced in section \ref{section:the_pinhole_camera_model}, the unknown model parameters are the coefficients of the camera model matrix K, which will be solved by the calibration step. 

Ideally, the calibration technique used should estimate the camera parameters in an unbiased manner. If a lens distortion model is added to the calibration stage, the calibration corrects image distortions produced by a real lens system, such as barrel and pincushion distortions, too. A MATLAB Camera Calibration toolbox will be introduced next, which was used to calibrate the stereo camera heads.

\subsection{Intrinsic Camera Calibration}
\label{subsection:intrinsic_camera_calibration}

The intrinsic camera calibration step will further extend the pinhole camera model introduced in section \ref{the_pinhole_camera_model} with a simple lens distortion model. A model is required to correct for lens distortions so that the image rectification step does not fail due to distortions. First, the perspective projection according to equation \ref{} would be written open as
...
where the distortion model is calculated for $x_d$ and $y_d$ following the calibration procedure described in \ref{Heikkila97}. The intrinsic calibration parameters were computed using Jean-Yves Bouguet's Camera Calibration Toolbox for MATLAB. The calibration of the HIMMELI cameras follow the notation and the workflow of Heikkilä's paper, and the camera calibration toolbox provides its implementation\ref{Fetic12}. Bouguet's camera calibration toolbox provides lens distortion models up to a 6th order polynomial, but in the calibration of HIMMELI only a 4th order model was needed, leaving the 5th distortion coefficient in the 5x1 distortion coefficient vector $kc$ zero. Thus, the distortion corrected normalised point coordinates $x_{d}$ and $y_{d}$ follow:
