...
To combat the bounding volume being reported as a function of the load object orientation in the world frame, an oriented bounding box OBB can be used. The 3d OBB rotates its perimeter along the load object main axis as seen in Figure \ref{fig:aabb_obb} b). An OBB bounding volume minimises the bounding volume optimally and makes it invariant to load object rotation in the world frame.

The eigen vectors of the load object point cloud can be computed, thus, the OBB perimeter can be oriented along the largest eigen vector in a right-hand normalised coordinate system. While orienting the main axis along the largest eigen vector does not optimally minimise the bounding volume, it does make the bounding volume more invariant to changes in the load object orientation in the world coordinate frame. The OBB orientation can be done using PCL \emph{moment of inertia and eccintricity based descriptors} module, which is only supported by PCL versions 1.7 and higher. Since ROS Groovy did not support PCL version 1.7, the OBB computation was not used.
