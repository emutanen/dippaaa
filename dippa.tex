...
Ideas of the modern container cargo handling process started only after the second half of the 20th century. Before that the only known way of sea transport for goods was packing items in barrels, sacks and pallets. It was cumbersome to load and unload a ship filled with such small units, it was not unusual that ships spent more time in docks than at sea because of inefficient manhandling of small cargo.
...
Measuring goods while they are being moved and processed at the industrial site is the very basis for all quality control in manufacturing industries: radio frequency identification tags are used to identify and control a batch of products in bulk manufacturing, video systems measure process quality in a paper factory, and spectral analyzer measures the concentration of foreign substances in a soda bottle washing process, to sum up a few. While it is possible to find examples of fast, automated, optimized, profitable, and safe factory manufacturing workflows, many industrial ventures still rely on manual movement of a product on a factory floor. Such manual operation can suffer from non-optimized route selection, operator fatigue, damage to goods in collisions, safety issues for other workers, and other problems due human error. Some industries are not even profitable unless a certain speed of moving goods safely is reached, which applies for e.g. forestry log picking and tree felling work. To improve things a bit, a real-time description of the load object and its environment could be available, so that it would be possible to prevent unwanted events using the installed crane systems with some perception sensors and software.
...
The overall approach of accurately knowing where things move can bring automated safety and extended information for the remote operator. For example, tracking objects with known locations, dimensions, and speeds, it is possible to add collision avoidance scheme in an environment where such safety feature never existed previously. The biggest non-safety related features are the optimized routing and movement speeds of the load item using computer planning for routing. During the life cycle of the crane installation such a feature can reduce vast amounts of used energy, possibly increase movement speeds, and sometimes cut down need for service. Optimized route planning has become available in modern process crane PLC devices, which means that it is possible to test route planning right away in the best case scenario. %reference for energy savings?

Load object measurement can additionally be used for object detection and tracking, and maybe even for security access surveillance when the factory is shut down. Furthermore, the data can provide additional statistics, for example, average object size that the crane has lifted recently. Size and weight statistics can help track the crane usage and lead to improvements in warehouse layout, or help a factory manager decide upon investing in a larger manufacturing floor if the capacity is at a limit.

The focus of this work is on measuring the dimensions of industrial goods being lifted and moved in an industrial site using a process crane. Industrial goods, such as beams, engines, large coils, drums of dangerous goods, and pallets of stacked items are expensive, often unique one-time orders that can cause serious damage if in collision with other objects. Even with wireless remote controllers, anti-sway pulley systems, and countless other improvements installed in the crane systems there is still room for improvement in the manual operation mode. It is definitely possible to improve the safety and speed of handling of heavy items using process hall cranes or forestry cranes with operators.

\section{Overhead Cranes}
\label{section:overhead_cranes}
% big picture
A glimpse in the not so distant history reveals a development of the overhead crane, a hoisting system more commonly known as the bridge crane or the process crane. First mass manufactured overhead crane systems were commissioned in 1876 in England, and they were powered by steam engines in the beginning of the industrial revolution. Now, 138 years later the overhead crane is widely used in different industries for efficient process lifting and service duties - using electricity. The development of the electric overhead traveling crane (EOT) has introduced the modern overhead crane as we know it. It is electrically powered and often used in heavy process industry, and in custom industrial manufacturing.
...
The overhead crane may be installed in the factory space before the process machinery is in its place, so that the actual construction of the factory is done with the newly installed crane. The installation cost of the crane in a new industrial manufacturing plant can be approximately same as the rental cost of construction cranes. Monetarily, it makes sense to install a crane to an expensive industry plants since it will be later required for service operations of the process machines, too. In custom manufacturing and warehouse solutions, the overhead crane is often installed inside a warehouse on rails, and sometimes outside the warehouse, supported with beam walls for lifting in e.g. dry docks and train depots.
% thesis focus on overhead cranes 
In this thesis, the main focus of load object dimensions measurement is on custom manufacturing, and warehouse case load objects, where the carried object size is previously unknown and may change during the lifting operation. Since each item lifted with the crane should be a unique object that was ordered by a customer, or at least a non-standard object, it is valuable knowledge to understand what size the load object is to the software system that is moving it in the warehouse.

. For heavy process industry, it is probably not interesting to measure load objects (containers) whose size is known, and at least, the size is standard even if not all load objects are the same size.



Thus, it is not interesting to measure dimensions of molten metal cylinder since it is known to stay constant, and can be easily measured 


Also for practical reasons, the heavy process industry applications are not in focus of this thesis. 

On one hand measuring dimensions of a molten steel cylinder is not 


which is why the developed load object measurement in this work is done with 


%structure of a single overhead crane
Overhead cranes have a beam-like bridge installed between two rails. The bridge spans the gap between the rails and travels from one end of the rail to the other. Effectively, a polyhedron shaped crane working area (portal of the crane) is formed underneath the rails encompassing most space between a factory floor and the height of the installed crane. It can be easily deduced that an overhead crane has 3 degrees of freedom (DOF). The traveling bridge produces the first DOF (X axis), and the second DOF comes from the moving trolley mechnism that moves perpendicular to the traveling bridge direction (Y axis). The third, final degree of freedom comes from the hoisting mechanism that covers the Z axis direction movement of the end effector tool.
...
, thus, a 3d working area 


. The bridge itself is traveling on the rails from one end of the rail to the other, effectively forming a working portal underneath the installed rails where lifting can be done. 



In this work, we are interested in measuring lifted cargo dimensions of small to medium heavy lifting 



the operator controls it most often using a remote control desk, a control pendant, or a wireless hand-held controller. The interest in measuring the dimensions of an overhead crane load 
...
\section{Environment Acquisition Techniques}
