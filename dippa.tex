...
\label{section:camera_configurations}
...
There are some conditions that must be fulfilled in order to find the correct solution. First, the ground plane must contain the largest number of points in a planar fashion found in the dataset. If not, then another plane will be found by the algorithm, for example, a large warehouse wall can be found in the RANSAC search. Secondly, the plane response must be within the selected error threshold. In the bird's eye views for both tested crane environments some points were not removed that fell outside the error threshold due to high depth estimation error and curving of the floor-wall interface as described later in Section \ref{subsubsection:findings_from_birdseye_viewpoint_datasets}. Similarly, the ground plane estimator will fail to remove grounds that are not planar, e.g. rolling hills or mounds of dirt in the ground.
...
The cylinder model estimation was used in finding cylindrical load objects from the 3d point cloud data. In the rotary boom crane case the load objects consist of a single log (or multiple logs), which can be approximated as a single cylinder. The model estimation uses exactly the same technique as was described for plane model segmentation in Section \ref{subsubsection:parameter_estimator_for_ground_plane_removal}, the only difference is that the model type is replaced with a cylindrer implicit parameter model using type pcl::SACMODEL\_CYLINDER. The tutorial code for implementing a cylinder model segmentation in PCL code can be found from the pointclouds.org website\citep{CylinderModelSegmentation14}.  
...
Some problems in cylinder segmentation were encountered because the load object point cloud contains the rotary crane boom. The estimator easily fitted the cylinder model on the crane boom and not on the load object, which was an incorrect result. To prevent selection of wrong data, a set of simple heuristic checks were implemented in the software.
...
If a verification check reported that the location of the found load object is not correct, then the cylinder segmentation was being iterated until the verification check reported a correct cylinder (or a cylinder not found). To make the RANSAC segmentation find new cylinders, the previously segmented incorrect points are being deleted from the data, and the RANSAC search is imposed again on the remaining points. The heuristics used are evaluated in Chapter \ref{chapter:evaluation} and discussed in more detail in Section \ref{subsection:log_measurement_results}.

The results of the cylinder model estimation were good since the estimator found the correct cylindrical load object nearly 100\% of the time when the logs were lifted in the air. The cylinder segmentation returned a correct result even for a load object that was on the ground merged with other logs approximately 50\% of the time. It is not possible to detect the number of logs carried in the boom crane using the RANSAC model estimator, but the bounding volume can be computed for multiple logs. The radius of the cylinder was estimated slightly too large typically, which is acceptable in bounding volume computation.
