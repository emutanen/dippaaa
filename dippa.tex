...
% big picture
A glimpse in the not so distant history reveals a development of the overhead crane, a hoisting system more commonly known as the bridge crane or process crane. First mass manufactured overhead crane systems were commissioned in 1876 in England, and they were powered by steam engines in the beginning of the industrial revolution. Now, 138 years later the overhead crane is widely used in different industries for efficient process lifting and service duties, just using electricity this time. The development of the electric overhead traveling crane (EOT) has introduced the modern day overhead crane as we know it. It is powered electrically and often used in heavy process industry and in small-scale manufacturing sites.
% medium picture, application
Overhead cranes are used for medium to heavy lifting, in heavy process industry, and also in warehouse solutions. For example, in steel mills, heavy lifting is needed when cylinders of molten steel or other metal are carried to the next process station. Also the moving of finished products such as large iron beams requires heavy lifting that only an overhead crane can carry out. 

The overhead crane is most often installed inside a warehouse for lifting purposes, or sometimes outside supported with beam walls for small-scale lifting use in e.g. dry docks and train depots. Naturally, for each desired application a different solution exists, and the looks of the overhead cranes may vary a lot, but the operating principle stays the same.
...
Overhead crane installation cost in a new industrial manufacturing plant can be approximately same as the rental cost of outsourced construction cranes, thus, it makes sense to install overhead crane systems to heavy industry plants to help install and service heavy equipment.
