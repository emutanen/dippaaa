...
Testing of the implemented software proofs that the selection of ROS as the communication platform enabled stable heterogenous PC networking with Himmeli platform, on which any data could be relayed from any software component to other. The drawbacks that this setup had, were

\begin{itemize}
% item separation length in a list
\setlength{\itemsep}{0pt}
\item The ROS network introduces delays, as expected
\item Data bandwidth for topics is capped to a maximum at some limit posed by the software or the hardware
\item 
\end{itemize}

Still, ROS is highly customizable, and a great number of readily available visualization, configuration and debugging tools were used to help get the comms running. 
%, such as rviz, rqt_graph, rqt_reconfigure, rosparam, roswtf, and rqt_logger.

ROS enabled the project to use heterogenous PC networking to communicate with the HIMMELI platform, and it provided large community-based support with debugging tools, and almost real-time networking using topics and subscribers.

The biggest risk with using ROS was the fact that the project team was missing ROS programming skills, and it was known tha the learning curve is steep for ROS systems. The risk 
