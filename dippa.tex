...
After using the NaN removal filter, the data becomes unorganised, which drops the number of points needed to process from 442368 points to a lower number of approximately 250000 points. Furthermore, the point cloud is being downsampled to approximately 2.2\% size of the original number of valid points using the PCL voxel grid filter (in pcl::VoxelGrid class). 

The voxel grid filter outputs a downsampled point cloud by approximating multiple data points in a region with their centroid. The filter computes a grid of voxels, and all points residing in a single voxel are replaced by their centroid or the center of the voxel. The implementation uses the centroid approach, which is slower than the voxel center approach, but this way the underlying 3d surface is being represented more accurately. \citep{VoxelGrid14}

The voxel cube size parameter controls the amount of downsampling in the voxel grid filter. The used value in the work was 120.0 (units in millimeters), and it was found using a slider that changed the value in real-time in the software system. The voxel grid leaf size has an effect on the object segmentation process, which is why the parameter was visually inspected to work . 
