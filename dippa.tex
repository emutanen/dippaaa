...
Monocular camera calibration is the process of finding unknown camera model parameters e.g. the focal lenght components for a single camera system. Using the pinhole camera model introduced in section \ref{section:the_pinhole_camera_model}, the unknown model parameters are the coefficients of the camera model matrix K, which will be solved by the calibration step.

Ideally, the calibration technique used should estimate the camera parameters in an unbiased manner. The calibration corrects image distortions produced by lenses, such as barrel or pincushion distortions. 
