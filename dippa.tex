...
The pinhole camera model maps the perspective transformation from 3d space to a 2d image plane using simple geometry. The performed mapping transforms the three-dimensional object from real space $\mathbf{R}^3$ into a two-dimensional projection according to $\mathbb{R}^3 \mapsto \mathbb{R}^2$. The depth component Z is lost in the transform, which is natural for any projective camera models. The transform is irreversible, which means that the original 3d coordinates cannot be recovered from a projective 2d image. The pinhole camera model is introduced because it is the simplest approximation that can be used in computer vision applications. Additionally, the openCV implementation in chapter \ref{chapter:implementation} uses a pinhole camera model for computation\citep{Sonka07, OpenCVWeb}.
...
The pinhole camera geometry is introduced in Figure \ref{fig:pinhole_camera}. A 3d object located at a distance of $p$ from the camera lens (pinhole) is imaged upside down on a 2d image plane $\pi$. An optical axis $I_o$ (dashed line in Figure \ref{fig:pinhole_camera}, positive Z direction in the camera frame $O_c$) lies perpendicularly to the image plane pointing towards the object. In a pinhole camera, the object and its real projected image are related by the thin lens equation
...
where f is the focal length of the thin lens. An upside down imaged is formed on the image plane as long as the 3d object or its image are not nearer than $f$ units from the focal point of the lens. Using similar triangles it can be shown that the point P is projected onto the image plane according to
...
In homogenous form, a projected point Q' = ($x'$,$y'$,$z'$) can be further formulated as

\begin{equation}
x' = f\frac{X}{z'},\~ y' = f\frac{Y}{z'},\~ z' = Z
\label{eq:image_projection}
\end{equation}
...
Point P can be written in homogenous matrix form as well as $P' = (X,Y,Z,1)$ so that the projection becomes a linear matrix operation
...
where C is a 3 x 4 matrix called the camera matrix, and Q' and P' are the homogenous image plane projection and 3d object surface point coordinates, respectively.

\subsection{Image Formation}
\label{subsection:image_formation}
