...
Stereoscopic perception is a natural phenomenon first discovered in human and animal vision systems. Stereoscopy in human vision is based on stereopsis effect, which means perceiving depth in a scene using binocular vision with two monocular vision eyes. The same depth perception can be programmed to a computer, but instead of eyes, cameras are used. Instead of the brain, a computer is used to compute depth information for an image pair. A calibrated two-camera system that is capable of extracting depth information from a scene is most often called a \emph{stereo vision system}, or a \emph{3d vision system}.

While humans perceive and estimate object sizes with the help of their previous knowledge of the world, and their implicit stereo vision calibration by the brain, a machine vision system must be carefully calibrated to output metric measurements of the world. For simplicity, in this thesis a single-time calibration is considered and no machine learning techniques are used to improve the calibration at a later time.

After capturing a pair of images and processing them, it is possible to request a 3d coordinate value for almost any feature found in one of the images. In this way, the machine vision system can deliver coordinates for many object features in a scene simultaneously, which is quite impossible for a human to do. In this way, by selecting visible points from the load object from a 2d image the coordinates for those feature points are computed by the computer. With further processing, the load object dimension measurements may be recovered with some constraints. Transforming the 2d image into a partial 3d feature model is important since the measurement is ultimately done by requesting depths of points of interest and measuring their difference along all the axes.
