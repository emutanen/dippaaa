...
Log measurement using simple cylinder fitting was implemented successfully, and the resulting found log object can be seen in Figure \ref{fig:log_detected}. The orientation of the log was found approximately within a 10 degree error window 95\% of the time, but no exact statistics were computed regarding correctness of the extracted orientation. The position of the log object includes a translation error in the direction of the cameras that is caused by the missing data points from the back side of the log. The amount of erroneous translation towards the cameras was not measured. Both the translation error and the orientation error affect the bounding volume measurement output, but the effect is not very big.
...
Better heuristic checks for correct load object selection is suggested as an improvement in the log object extraction and measurement. Currently, the extracted cylinder is the actual log object most of the time, and selecting parts of the crane boom is prevented using simple constraints. Still, it is possible to select a wrong load objects that is located below the end effector, for example, data generated by a pile of logs. Now, it is being checked whether the extracted cylinder is located above the end effector or not. The extracted cylinder will be discarded as part of the crane boom if the data is located above the end effector location.

Two checks that would improve the current performance significantly would be 

\begin{itemize}
\setlength{\itemsep}{0pt}
\item to check that the shortest distance from the end effector to the closest point along the cylinder main axis does not exceed a pre-defined maximum threshold 
\item and that the distance between the centroid of the segmented cylinder and the end effector do not exceed a pre-defined threshold.
\end{itemize}

For example, if the distance between the candidate cylinder and the end effector is too large, then the candidate should be rejected. An improved load selection algorithm would be easy to implement, but due to limited time it was not further developed.

In Figure \ref{fig:load_detected1}, the stereo parameters and object segmentation parameters were not tuned optimally. Still, the segmentation was quite robust in presence of missing data, noisy 3d data, and non-optimal tuning of parameters. Especially the OpenCV stereo calibration that is described in Section \ref{section:stereo_calibration} will estimate good parameters with little a priori information and work a lot towards robust functionality.

The most significant problem in log object measurement 
