...
The most significant problem in log object measurement was the grouping of multiple objects in the load object point cloud. While the cylinder extraction does a good job of extracting the correct cylinder, it remains unknown whether the object segmentation step is needed at all in forestry crane case since objects tend to group in a single large point cloud most of the time. The load object point cloud usually contains the crane boom, the end effector tool, and the log picked up. Sometimes, the load object point cloud additionally contains objects lying on the ground.

It could be assumed that the load objects are always found below the end effector tool, which would limit the number of points needed to process, but it can be easily shown that in normal operation the log is often partially seen above the end effector tool. In forestry crane test site, the long logs did easily tilt and reach above the end effector tool location when the operator picked them up. Consequently, the bounding volume does reach above the end effector height, too. It was decided that in order to avoid accidental cutting of the load object after segmentation  it cannot be assumed that the load object is located only below the end effector tool. Consequently, only object centroid proximity information should be used to decide whether the selected load object is actually grabbed by the end effector tool or not.
