...
The oriented bounding box (OBB) is similar to the axis-aligned bounding box with one difference: the main axis of the bounding volume coordinate system is oriented along the main axis of the load object. This way, a minimum bounding volume is achieved with the object optimally filling the cubic shape of the OBB. Thus, the bounding volume of the load object is minimized and the object space axes orientations are produced.

The PCL library versions 1.7 and higher support a version of the oriented bounding box computation using moment of inertia and eccentricity based descriptors. With these descriptors the eigen vectors of the point cloud object can be computed, and the bounding box can be oriented along the largest eigen vector in a right-hand normalised coordinate system. The OBB along the major eigen vector axis does not guarantee minimality of the bounding volume, but the method does make the bounding volume computation more invariant to changes in the load object orientation in the world coordinate space. Unfortunately, the version of ROS Groovy used in the project does not include the moment of inertia and eccentricity descriptors required to compute the OBB. Consequently, future versions of the software running ROS Hydro with PCL 1.7 and higher can improve the bounding volume accuracy by implementing an oriented bounding box computation that replaces the AABB bounding volume computation currently used. 
