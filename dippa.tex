...
\section{Incentives For Measuring Moving Materials}
\label{section:incentives_for_measuring_moving_materials}

Measuring goods while they are being moved and processed at the industrial site is the very basis for all quality control: radio frequency identification tags are used to identify and control a batch of products in bulk manufacturing, video systems measure process quality in a paper factory, and spectral analyzer measures the concentration of foreign substances in a soda bottle washing process, to sum up a few. While some fast, optimized, profitable and safe system examples are found, many industrial ventures still rely on manual movement of a product on a factory floor. Manual operation can suffer from non-optimized route selection, operator fatigue, damage to goods in collisions, safety issues for other workers, and other problems due human errors. Now, if a real-time description of the load object and its environment was available, it is possible to prevent such problems using the installed crane systems itself.

A semi-automatic, safe process crane working area is the desired target that a description of the environment could provide. Currently, to the knowledge of the author, there are no commercially available systems that can provide a real-time description of an environment that integrates to a crane programmable logic controller (PLC), but it is a grand vision to start out with. The first step in realising such a system is to provide a measurement or a simple model of the load object and start aggregating other descriptions after PLC integration. To start with in later chapters, a perception sensor platform is used that can provide geometry knowledge of the process crane working area, and generates the best possible geometry description of the load object for the use of crane PLC computer. Optimized route planning has become available in modern process crane PLC devices, which means that it is possible to test such a system on a real crane with a provided measurement. 

Other usages for measuring objects in a process crane working area can be object detection and tracking, and night time security access control. Further external measuring of moved items can provide additional statistics, for example, a median moved object size value. Size and weight statistics can help track the crane usage and lead to improvements in warehouse layout, or help a factory manager decide upon investing in a larger manufacturing floor.





accurate description of the moved item and its environment could provide. This approach can bring extended safety with collision avoidance and real-time information for a remote user on currently moving goods and poeple. The main non-safety related feature is the optimized movement of the load object, which can save a lot of time and energy during the life cycle of the process hall crane.

Industrial goods, such as beams, engines, large coils, cylinders of dangerous goods, and pallets of stacked items are expensive, often unique one-time orders that can cause serious safety issues if collided with other objects, or in worst case, people. 



A lot of functionality has been added to the crane to help the operator , for example, wireless remote controllers for freedom of movement, anti-sway pulley systems, and others. One problem is that while the operator can move an item safely to its destination, 
...
On the other hand, some industries are not quite profitable unless a certain speed of moving goods is reached, which applies for e.g. forestry log picking and tree felling work.
...
would help the operator to make movements safely


an external measuring system should provide in a non-automated working area. The 
...
In a highly automated shipping terminal the movement of goods is optimized and the 

The focus of our work is on measuring the dimensions of industrial goods being lifted and moved in an industrial site using a process crane. 




%from e.g. a dry storage unit to a storage space
...
Stereoscopic perception is a natural phenomenon first discovered in human and animal vision systems. Stereoscopy in human vision system is based on stereopsis effect, which means perceiving depth in a scene using binocular vision. Similarly, by using two monocular cameras in a binocular setup it is possible to not perceive, but quantifiably measure depth using a calibrated camera system. %reference?

This kind of depth measuring machine vision system is most often called a \emph{stereo vision system}, or 3d vision system.
3d vision tasks are used in many engineering applications is generally seen as a problem of 3D scene reconstruction, object property understanding, or a visual control task of minimization, depending on the scientific application at hand \cite{Sonka07}.
