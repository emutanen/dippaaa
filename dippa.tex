...
The stereo correspondence problem is a general multiple camera system problem answering whether multiple images contain a same 2d feature or not. Figure \ref{fig:rectified_image_pair} displays a pair of images seen from different viewpoints, and the correspondence problem solves where the locations of the lifted load object are seen in both pictures relative to each other. People are good at solving the correspondence problem - clearly the same  box can be seen in both images in Figure \ref{fig:rectified_image_pair}, which is a solution that a computer system will not solve easily.
...
In a 2d image, a feature is a 2d response of a 3d object that is formed on the image plane according to the camera model described in Section \ref{section:the_pinhole_camera_model}. Since different cameras have different viewpoints of the same 3d object, the feature response of the object is not exactly the same for each viewpoint. 
...
Some problems in cylinder segmentation were encountered when the load object point cloud contained a crane boom itself. The estimator easily fitted the cylinder model on the boom and not on the load object, which was an incorrect result. To prevent selection of wrong data, a set of simple heuristic checks were implemented in the software.
...
If a verification check reported that the location of the found load object was not correct, then the cylinder segmentation was being iterated until the verification check reported a correct cylinder (or a cylinder not found).

To make the RANSAC segmentation find new cylinders, the previously segmented incorrect points are being deleted from the data, and the RANSAC search is imposed again on the remaining points. The heuristics used are evaluated in Chapter \ref{chapter:evaluation} and discussed in more detail in Section \ref{subsection:log_measurement_results}.
