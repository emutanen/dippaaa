...
back to 3d world coordinates, which solves the 3d reconstruction problem. The disparity map generation output quality depends on the selected block matching algorithm, and on the selected disparity tuning parameters. The disparity map generation parameters were tuned using a visualisation that was controlled with tuner sliders. The parameter tuning is discussed more in Section \ref{section:parameter_tuning_evaluation}.
