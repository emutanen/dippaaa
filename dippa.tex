...
A camera that views the external three-dimensional world records a two-dimensional image, thus, one dimension is reduced in the process of camera capture. Camera calibration is needed in order to extract the third dimension that was lost in the recording process. The goal of the calibration is to estimate two sets of parameters: the intrinsic parameters and the extrinsic parameters of the camera.  The extrinsic parameters relate two different reference frames to each other with a transformation that contains a translation and a rotation. The extrinsic parameters are used to represent a point in space measured by the second camera in the first camera's reference frame coordinates. Thus, the extrinsic parameters can relate two viewpoints of the stereo camera setup to each other enabling the depth map computation.

\subsubsection{Intrinsic Camera Parameters}
\label{subsubsection:intrinsic_camera_parameters}

Intrinsic camera parameters project points in space from camera reference frame to the image plane taking distortion models and the qualities of the sensor array into account. The intrinsic parameters are needed to understand the projection model of the actual camera system. There are in total 5 different intrinsic parameters that are 
...
\end{empheq}
.\par
The parameter r seen in equation \ref{eq:normalised_distortion_corrected_pinhole_projections} is just short for $r^2 = x^2 + y^2$. The tangential distortion coefficients calibrate decentering of the image and other defects in compund lens structure. The radial distortion coefficients corrects a barrel or pincushion distortion depending whether a telephoto lens system or wide angle lens system is used. 


A more in-depth analysis of the effect of the term may be found in the original publication by Brown \ref{Brown}
...
A camera model parameters whose amount depend on the selected camera and lens distortion models, and the number of calibration images used.
...
Calibration corrects image distortions produced by lenses, such as barrel or pincushion distortions.   
