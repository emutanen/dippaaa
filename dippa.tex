...
In the overhead crane case, the decision to use a proximity based load object selection from a pre-defined coordinate (instead of a 2d image tracker) was far from perfect, but it worked, and in the end no information about the end effector location was required to select and measure the load object. It was guessed that a load object would be found near the center of the image, which was only approximately true for the top-down view datasets. For example, in some bird's eye viewpoint datasets, the load object can be seen in the lower part of the image far away from the center. Because of such assumptions, often wrong objects were reported as the load object, which can be seen as clutter in the dataset visualisations.
