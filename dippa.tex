...
\section{Collision Avoidance In Crane Environment}
\label{section:collision_avoidance_in_crane_environment}

Industrial goods e.g. beams, engines, large coils, drums of dangerous goods, and pallets of stacked items are expensive, often unique one-time orders that cause damage if in collision with other objects, which is why an automatic route planner that avoids collisions is an interesting prospect in a factory. Collision avoidance technology could automatically keep lifted products intact, and warn the operator of safety hazards and engage safety measures if needed. New overhead crane systems do not currently have any collision detection features. If the load object is carefully measured, its location and orientation is known  at all times, and also a model of the environment is known, then a collision computation can be used to detect future collisions before they happen.\par
...
Latest additions to overhead cranes are features like load anti-sway systems, wireless remote controllers, and other improvements.

installed in the crane systems there is still room for improvement in safety of the crane working area. It is definitely possible to crash to a wall or in an unfortunate event, to other working people with a 3-ton metal case, or even heavier objects, which in the case of collision can cause structural damage to buildings, break goods, or injure co-workers.
