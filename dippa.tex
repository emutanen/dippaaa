...
Structure from motion (SFM) is a differential technique that uses only a single camera and a sequence of images to reconstruct 3d geometry from a scene. Many different similar techniques exist, for example structure from shading, 
...
SFM can extract information from a video source using image sequence properties, such as the motion field. The motion field 
