...
\section{Industrial Goods In Smart Crane Environments}
\label{section:collision_avoidance_in_crane_environment}

% an automatic route planner that avoids collisions

Industrial goods e.g. beams, engines, large coils, drums of dangerous goods, and pallets of stacked items are expensive, often unique one-time orders that are being lifted and transferred inside a warehouse. If in collision with surrounding environment, they may damage or break other products, which is why a smart crane environment that avoids collisions in a warehouse is an interesting design for a factory. Collision avoidance design is based on automatic route planner that has a model of the environment, and understands 

With current generation manual overhead cranes it is definitely possible to collide the load object with other objects that lie on the factory floor on on the shelves of a warehouse. A collision avoidance system would prevent collision based damage in the environment, and it also detects people and tries to prevent any crane operation based injury to factory workers.

Collision avoidance technology could automatically keep lifted products intact, and warn the operator of safety hazards and engage safety measures if needed. New overhead crane systems do not currently have any collision detection features. If the load object is carefully measured and its geometry is known at all times, a collision computation with a known environment model can be used to detect future collisions before they happen. The added value is potentially good, which is why the measurement of the load object is worth researching.


Latest technical additions to overhead cranes have been features like load anti-sway systems, single motor hoisting mechanisms, wireless remote controllers, and other improvements that add value for the crane operator and did improve safety and speed of overhead crane operation. Still, 

The change from manual operation to semi-automatic operation or fully automatic operation would possibly increase speed and safety, but also affect energy-efficiency, product life cycle, and maintenance needs positively. Some automatic overhead cranes have been built, but usually they move bulk items, such as sand, steel, or other process-like material that is moved in environments where people are not working.
 With these potential improvements in mind, a dynamic model of the environment is needed for realising such a system. The first step is to test the load object modeling using a machine vision system, and develop the system further after 



From a technical point of view, it is difficult to perfectly track an object continuosily in space and time, but if an understanding of the model of the object, and its qualities are formed, then a request of the state of the model can be polled from a computer system at times when the model state is needed for decision making. With such information, tracks of objects can continuosly provide information about object's location, dimensions, and moving speeds. With this model information it is possible to add new features to a crane working environment. 



%The described collision avoidance safety feature can be realised if an understanding of location, orientation, dimensions, and movement of the load object and its surroundings are accurately known. 
