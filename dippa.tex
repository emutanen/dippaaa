...
The goal of this work is to measure the dimensions of industrial goods being lifted and moved in an industrial crane setting with the help of a stereo camera based machine vision system. The work is carried out and analysed mainly for two different environments: an indoor overhead crane process hall, and an outdoor rotary boom crane on a forestry machine. The tested scenarios are detailed more in Section \ref{subsection:use_cases} "Use Cases".

The overhead crane load object measurement is the primary objective that will be introduced in this work. The forestry crane load object measurement is a secondary objective that investigates the applicability of the same machine vision system in a log picker truck setting.

The project affiliated with this master's thesis was the GIM research group's FAMOUS project whose deliverable the actual software client for load object measurement was. As such, another primary objective is to implement the load object measurement software that is easy to integrate into other software solutions and supports Robot Operating System on a network of PCs.

A tertiary objective regarding hardware was to keep the costs of the load object measurement system significantly lower than the price range of a LIDAR based solution. A tertiary objective regarding software was to make the load object measurement output rate nearly in real-time, or at least 5 measurements per second.

\chapter{Perceiving Environment In 3d}
\label{chapter:perceiving_environment_in_3d}
...
\section{Analysis On Overhead Crane Load Object Measurement}
\label{section:analysis_on_overhead_crane_load_object_measurement}

In the overhead crane case, the performance of the load object measurement software was tested extensively using two selected viewpoints: the top-down viewpoint and the bird's eye viewpoint. On the basis of all findings in Chapter \ref{chapter:evaluation} the top-down viewpoint performed faster, and resulted in a successful load object measurement more often in different locations inside the test hall environment than the bird's eye viewpoint. In light of all findings and analysis the top-down camera view is recommended for load object measurement purpose in an overhead crane environment.

\subsection{Effects Of Selected Viewpoint}
\label{subsection:effect_of_selected_viewpoint}
...
In the overhead crane case, the decision to use a proximity based load object selection from a pre-defined coordinate (instead of a 2d image tracker) was far from perfect, but it worked, and in the end no information about the end effector location was required to select and measure the load object. It was guessed that a load object would be found near the center of the image, which was only approximately true for the top-down view datasets. Often wrong objects were reported as the load object, which can be seen as cluttered dataset visualisations. For example, in some bird's eye viewpoint datasets, the load object can be seen in the lower part of the image far away from the center.

\subsection{3d Geometry Verification}
\label{subsection:3d_geometry_verification}

3d data geometry was assumed to be correct with a calibrated stereo system, which is an assumption that may or may not be true. 3d data geometry was verified only for load objects by comparing the dimension measurements produced by the load object measurement software and the actual measure taken with a measuring tape. This was not enough to validate any other parts of the geometry in the surrounding environment.

A suggested improvement for environment 3d modeling is to install 3d spheres as control points in the environment before data capture. Then, the reconstructed geometry can be verified as a function of distance from the cameras by comparing the computed locations of the spheres with their actual locations. If the 3d data locations of the control points do not match their actual locations, a distorted dimension or a scale change could be detected. Since the emphasis on the thesis was not environment modeling the 3d geometry of the environment was not very carefully verified, and as long as the load object reports realistic dimensions, it can be assumed that a correct calibration results in a correct geometry.

The values produced by the software were not comparable with the actual measurements before the orientation of the sensor frame and rotation of the load object were taken into account. The axis-aligned bounding volume reported on average approximately 5-10\% larger values than the actual values of the dimensions, which is an acceptable result.

It was also deduced that a single measurement will not report the correct bounding volume, but it is a better idea to report the median or average bounding volume of a series of e.g. 10 to 15 measurements. Since the Himmeli platform can successfully deliver 4 frames per second without skipping frames, a load object measurement would take 3-5 seconds of time in total with the used software. The decision of how to interpret the constant flow of load object measurment data in the ROS network is left as responsibility of the software that subscribes to the provided data feed.

Since the used stereo cameras can deliver images at a steady rate of 30 FPS, the perception platform software was the bottleneck on the currently used system. Software operations can be sped up by making improvements in the point cloud data processing, or by removing visualisations from the load object measurement client. It was found out that updating the visualisations in the software requires most time, thus, removing visuals will speed up the point cloud processing when there is no need to use time on on-screen point cloud rendering. Also, the C++ implementation of the software could support more parallel programming and GPU implementations for selected point cloud computations. Finally, lower resolution cameras can be used, or the image ROI could be limited to only contain the image of the load object in order to reduce the amount of data and speed up the computations.

\section{Analysis On Forestry Crane Load Object Measurement}
\label{section:analysis_on_forestry_crane_load_object_measurement}
