...
Finally, the perception platform hardware suffered from minor problems, for example, one of the PC computers halted during heavy processor loads due to inadequate dissipation of heat from the processor. Another problem was that the left and right camera images were assigned for the cameras randomly, which caused the left camera image to be saved as a right camera image and vice versa. A swapping of left and right images caused failure in generating 3d data from the images. Currently, the only suggestion for improvement is to install a larger heat sink in the overheating processor core. 
