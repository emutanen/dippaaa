...
Geometry measurement of the load object is suggested to allow for new smarter automated features to overhead crane environments, such as route planning, collision detection, and collision avoidance. The incentives for developing such features are increased goods handling safety, and the need to improve easiness of operation for the crane operator. The load object measurement itself is not sufficient to enable the features, but it must be available in all the cases. 
