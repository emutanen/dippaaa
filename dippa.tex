...
%    \centering
%    \includegraphics[width=.3\linewidth]{harris_feature_detector_left.jpg}
%    \caption{\tiny Left rectified image.}
%    \label{fig:harris_features_left}
%\end{subfigure}%
%\begin{subfigure}{.5\textwidth}
%    \centering
%    \includegraphics[width=0.3\linewidth]{harris_feature_detector_right.jpg}
%    \caption{\tiny Right rectified image.}
...
Planar image rectification (stratification) is the process of virtual optical axis parallelisation for projective planar camera geometries done by software. Rectification transforms the image planes so that the same features seen in both stereo images are perceived on the same epipolar lines (see section Epipolar Geometry \ref{section:epipolar_geometry}). Since optical axes are perpendicular to the image planes, the $u$ and $v$ axes of the image frames are also being parallelised into a coplanar camera configuration. Besides planar image rectification, also cylindrical and polar rectification can be used for non-planar camera projective geometries.\citep{Pollefeys99}
...
A stereo camera pair sensors can never be fully parallel due to limits in mechanical alignment of the lens system and the construction of the sensor array. Software rectification compensates for such errors, thus, a very accurate positioning of the cameras is often not necessary. If a horizontal stereo pair cameras are shifted vertically, then some vertical disparity is introduced in the image features, which usually has only minor effect on the image rectification. Tilting one of the cameras in a horizontal stereo pair around the camera frame X-axis is compensated by the planar rectification process, but it is good to understand that the more the camera is tilted, the more interpolation will be included in the keystone correction. Furthermore, if the objects in the rectified image are located on the border of the image, the lens distortions will have a larger effect on the stereo matching. In general, the performance of the stereo pair is better with as parallel stereo setup as possible.  

A successful image rectification can be seen in figure \ref{fig:rectified_image_pair} where the corresponding feature points are found on the same horizontal lines. The correspondence search space is reduced to a single horizontal line in both images. One indicator of the goodness of the end result is the region of interest (ROI) values that are output by the rectification process. The larger the ROI is in the output, the more there is image detail left, and the image usable area loss (black margins) is minimised. A large ROI will give better results than a small ROI. 
