...
Also some new applications can be thought out that use the stereo camera platform that is used in this work (see Chapter \ref{chapter:implementation}). 
...
For example, training videos can easily be recorded for new operators, and records of events may be used in case of accidents. Furthermore, the load object measurement statistics could be used to keep track of the machine usage.

The load object measurement should add functionality to safety applications. A traditional safety application halts the crane process if people are detected in a dangerous work zone \cite{Raula11}. With the help of a collision control system, an adaptive safety scheme can be used to restrict the operation range of a machine, but not fully stop it.
...
This master's thesis was part of FAMOUS project. The research was conducted in GIM research group. The software client for the load measurement software was a deliverable of the project. As such, one primary objective was to implement a load object measurement software that is easy to integrate into other software solutions and that supports Robot Operating System (ROS) on a network of PCs.

An objective regarding hardware was to keep the costs of the load object measurement system significantly lower than the price range of a LIDAR based solution. An objective regarding software was to make the load object measurement client provide measurement data nearly in real-time, or at least 5 measurements per second.
