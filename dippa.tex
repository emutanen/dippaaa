...
In the modern world, industrial goods are being moved more than ever, but only few industrial ventures rely on automatic movement of a product on the factory floor using overhead traveling cranes. The approach to measure moving goods in crane environments can enable new smarter automated features to overhead cranes, such as route planning, collision detection, and collision avoidance features. The incentives for developing such features are increased goods handling safety, and the need towards more optimal energy consumption and longer crane life cycles.
    ...
    The perception platform data from real crane environment produced a deterministic response with some noise for load objects. The camera viewpoint affects th
...
Now, a comparison criterion may be formulated that relates the output value of the geometry measurement software and the actual measures of the large standard box.
