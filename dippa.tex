...
To combat the bounding volume being reported as a function of the load object orientation in the world frame, an oriented bounding box OBB can be used. The 3d oriented bounding box (OBB) rotates its perimeter along the load object main axis as seen in Figure \ref{fig:aabb_obb} b). An OBB bounding volume minimises the bounding volume optimally and makes it invariant to load object rotation in the world frame.

OBB will report the actual bounding volume more accurately than the AABB bounding volume, and it is invariant to rotations of the load object. Still, for load object measurement purposes, an AABB bounding volume is a good solution, and OBB bounding volume is left as an improvement suggestion in the current software solution.
