...
One finding about the effects of the angle of convergence was that with increasing toe-in angle, the resolution of the ROI in rectified image decreased. The lower resolution could be seen from the calibration files that reported the region of interest (ROI) of the rectified image in pixel values. OpenCV takes this change into account in the stereo rectification process, but as the rectified image becomes more and more distorted with increasing toe-in angle, the grid array available for reprojection becomes smaller, which decreases the number of points in the output point cloud.
...
The tuning of the BM parameters was done against offline data collected from all selected camera configurations.
High saliency image regions produce good results in block matching, and a rule of thumb would be: more visible 2d objects result in more correctly matched points in the region for the 3d point cloud.
