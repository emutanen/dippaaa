...
Some point cloud data quality metrics can be found in literature, such as spatial structure analysis methods and positioning value for registration schemes. Normally these methods are used in modeling of urban constructed environments and in airborne LIDAR mapping, which encourages to try their suitability for measuring constructed objects in indoor environments, too\ref{Feng08}.\par
In literature, also reverse engineering of shape primitives have been used for point cloud data quality verification in industrial settings. In industrial environments, a lot of box-shaped or cylinder-shaped objects can be found and be used for verification. Also, for any primitive geometric shapes it is fairly simple to quantify a goodness of fit value using geometric fitting schemes via least squares error minimization\ref{Fidera04}.
In spatial structure analysis, some real world feature qualities are measured from the environment, and the same qualities are then computed from the scene's point cloud response. Next, these measures are compared and it is possible to determine a goodness of fit -value that can be further processed into a goodness of data quality value. Of course, it is difficult to formulate a meaningful goodness indicator since point cloud data from stereo cameras always contains missing surfaces, measurement noise and errors increasing with more distant objects and other errors. Still, it is good to understand how much the reported measurement from the point cloud is off (in per cent) when compared to the actual object dimensions. \par
...
Positioning of point cloud features was not analysed in this thesis since no multiple view registration was used. Still, the spatial location verification with landmarks or beacons would have been useful since they could have been used to verify the correct scale of the point cloud, and correctness of all measured lengths. Since the data set environments did not contain any locations landmarks, the scale verification was not available. A further development suggestion would be to add landmarks objects (e.g. standard size boxes) in the environment and measure their locations prior to capturing the data sets to be able to do the spatial structure analysis.\par
In this thesis, such objects whose qualities are known a priori are called standard objects. For example, in the process hall data sets, a standard object titled \emph{large box} with dimensions of 1.52 x 0.94 x 0.92 meters was used. The large box object was constructed out of metal sheets, and it was a box-shaped, partially see-through mass weighing in 3.97 tonnes. In the forestry crane measurements, no standard objects were used, and as such, they can not be used for point cloud data quality control.\par
...
It is not meaningful to think of point cloud data quality if the scene is unknown, but for example a goodness of fit for a primitive shape is simple to quantify. 
