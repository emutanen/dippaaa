...
The image point that the camera outputs is the affine transformed \textbf{U}_c. Next, we must formulate a homogenous coordinate representation that allows to compute the affine transformation with a $3x3$ matrix multiplication.

We can represent a homogenous coordinate point in the image plane $\pi$ as 

\begin{equation}
\tilde{u} = \begin{array}{c}
U \\
V \\
W \end{array}
\end{equation}
...
where the presentation equals \ref{equation:projectedpoint} when 
