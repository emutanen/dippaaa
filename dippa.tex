...
Before the object segmentation results in Figures \ref{fig:downsampling} and \ref{fig:object_segmentation} are possible, the ground plane must be removed from the original data (see ground plane in Figure \ref{fig:object_segmentation} a)) via using a parameter estimation technique. 

If the data points belonging to the ground plane are removed, then only the load object and other items in the environment will remain in the point cloud data. Parameter estimation finds models from datasets, and in this case, it is known that a planar model (in 3 dimensions) is needed, which can be implicitly formulated as 
