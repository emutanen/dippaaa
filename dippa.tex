...
FLIR & Forward Looking Infra Red, a thermal camera setup \\
...
SSE & Streaming SIMD Extensions, an Intel optimized instruction set extension to the x86 processor architecture \\
TOF & Time-of-flight \\
...
\subsection{HIMMELI Sensor Platform}
...
\subsubsubsection{Occlusion Issues}
...
\chapter{Use Case Configurations and Use Cases}
...
A lot of time was used for working on the ROS implementation of the load detection software. The software has a simple backbone implementation thanks to the well-documented readily available OpenCV and PCL libraries. Some of the code runs advanced algorithms underneath, such as RANSAC model iteration, but these are not visible for the user. It would be possible to implement better algorithms for higher level functions, such as load detection computation and load geometric measurement computation, than what we are currently using. Due to limited resources we decided to keep the algorithms simple and instead realize a concept software that will run with a supported camera system. This way we have a working platform that can be used as a testbench for future upgrades and more advanced algorithms.  
