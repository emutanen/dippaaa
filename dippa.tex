...
Usually, a stereo camera setup is used for solving the problem of 3D information recovery from 2D images using knowledge about the camera geometry and the image content. We are mainly interested in machine vision as means of extracting point cloud data from a scene. Full recovery of the spatial 3D scene is not possible, because the stereo cameras only see the surface parts that project onto the image plane.

A full recovery of a 3-dimensional object model is not needed to solve the problem of load positioning. A computed point cloud will provide depth mapping for the visible surface, which we can further mask and process.
