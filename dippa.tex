...
\section{Software Design Review}

The design of the implemented load object position and orientation system did evolve during its implementation cycle. The original design was non-linear, component-based design running on C++ code base using the ROS system. After it was proofed that the Point Cloud Library processing could be used together with OpenCV processing, the integration of the ROS system started. The final implementation is more linear, component-based design, still running on ROS. Since the amount of data generated in the stereo cameras is quite large, the linearity of processing components enforces the design where data decreases as the information content increases.

From the design start, ROS was considered as the number one framework choice in terms of cross-platform communications. Other options considered were an own implementation of the UDP communication protocol, some other real-time operating system such as FreeRTOS, or an implementation with no networking capabilities at all. The other options were not as readily available for a full-fledged PC network in an industrial LAN setting as ROS was, and as such ROS was tried and tested for software development.

The biggest risk in using a ROS based network was the fact that the project team was missing ROS programming skills, 
, and it was known that the learning curve is steep for ROS systems. The risk realised in longer development times since ROS basics had to be studied first, but in the end the basic usage is not difficult at all. 


Testing of the implemented software proofs that the selection of ROS as the communication platform enabled stable heterogenous PC networking with Himmeli platform, on which any data could be relayed from any software component to another. The drawbacks that this setup had, were that 

\begin{itemize}
% item separation length in a list
\setlength{\itemsep}{0pt}
\item The ROS network introduces delays, as expected
\item Data bandwidth for topics are capped to a maximum at some limit posed by the software or the hardware
\end{itemize}

Still, ROS is highly customizable, and a great number of readily available software packages for visualization, configuration and debugging tools were used to get the network running as designed. These utilities were of tremendous help in configuring the network in different environments: in the LAN architecture of a real overhead crane hall, and in a local research network. 

It was also known that the learning curve for ROS programming is steep, so one risk in accomplishing a working online measurement system was actually missing knowledge on setting a ROS system up.
...
The evaluation of the load object measurement system has been done using offline data sets that were recorded from real use cases. Unfortunately, the online implementation was finished but not tested during the thesis writing, and as such the online performance is not included in this work.

ROS enabled the project to use heterogenous PC networking to communicate with the HIMMELI platform on wired LAN architectures, and it provided large community-based support with debugging tools, and almost real-time networking using topics and subscribers.
