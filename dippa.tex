...
The pinhole camera model maps the perspective transformation from 3d space to 2d image plane using a simple geometry. The performed mapping transforms the three-dimensional object into a two-dimensional projection ($\mathbb{R}^3 \mapsto \mathbb{R}^2$). The depth component Z is lost in the transform, which is natural for any projective camera models. The pinhole camera model is introduced because it is the simplest image formation device model that can be used for stereo photography purposes, and the openCV implementation in chapter \ref{chapter:implementation} uses this model for computation\citep{OpenCVWeb}.
