...
All the analysed overhead crane image data sets were captured in August 2013 in an actual overhead crane test environment in Finland as part of the GIM project. A total of 13 data sets were captured that contain different tests revealing how the dimension measurement of the load object can function in different scenarios. Environmental conditions did not change during the tests in the indoor process crane hall and the test were run in the same conditions accordingly.

A manual load selection approach was used since no end effector location service was available at the time of data set recording. Lack of 4d tracking of objects was compensated by selecting a pre-defined coordinate that selects the nearest (Euclidean distance) object as the load object. This solution was easy to implement, but the output of the load object selection selects wrong objects occasionally as load objects. Mainly, the effect of changing load object selection can be seen in the data visualisations, which seem cluttered because of sequential multiple different object measurements.
