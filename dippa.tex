...
Triangulation is a passive range acquisition mechanism that produces a range measurement from two cameras when a corresponding point shift information is known for a feature in a scene. Triangulation is the basic mechanism for most 3d acquisition techniques, which is why it is discussed before other techniques are introduced. In order to understand the functioning of a stereo camera based depth mapping technique 

Reconstruction by triangulation is the process of generating a depth map from two stereo camera images. If both intrinsic and extrinsic parameters of the camera are known, the reconstruction problem can be solved unambiguosly producing absolute coordinates in the world coordinate frame. As a simplification, two other cases would be that only intrinsic parameters are known, a
